    ...
    if (type === "image") {
        ...
        try {
            ...
            if (data.data && data.data.length > 0) {
                ...
                resultDiv.innerHTML = "<p>لم يتم استلام صورة. تحقق من صحة المفتاح أو حاول لاحقًا.</p>";
            }
        } catch (error) {
            console.error(error);
            resultDiv.innerHTML = "<p>حدث خطأ أثناء الاتصال بـ OpenAI. تأكد من الاتصال بالإنترنت وصلاحية المفتاح.</p>";
        }
    } else if (type === "video") {
        try {
            ...
            if (data.video_url) {
                ...
                resultDiv.innerHTML = "<p>لم يتم استلام فيديو. تأكد من وصف جيد وصيغة مناسبة.</p>";
            }
        } catch (error) {
            console.error(error);
            resultDiv.innerHTML = "<p>حدث خطأ أثناء الاتصال بـ RunwayML. تحقق من صلاحية المفتاح.</p>";
        }
    }
});
